...
def imprimirLista(lista):
    ...
    for lista in produtos:
        print(f"{contador}. {lista["Nome"]} | {lista["Estoque"]}")
        contador += 1
...
MENU DE OPÇÕES:
    ...
    5. Ver produtos pelo preço minímo
...
""")
    ...
    """)
        ...
        print("\nVER PRODUTOS POR ESTOQUE")
        print("Preço minimo é 60")
        ...
        for i, produto in enumerate(produtos):
            if produto["Estoque"] <= estoque_minimo:
                ...
                print("Número menor do que a quantidade do estoque.")
                break
    ...
    # Criar a funcionalidade 5, pesquisar pelo preço. O programa deve exibir todos os produtos que tem o preço maior ou igual ao preço digitado pelo usuário!
        ...
        print("\nVER PRODUTOS PELO PREÇO")
        print("PREÇO MINÍMO É R$ 2\n")

        preco_minimo = float(input("Digite o preço minimo: "))
        ...
        for i, produto in enumerate(produtos):
            ...
            if produto["Preço"] >= preco_minimo:
                ...
                print("Preço é maior do que os produtos no estoque.")
                break
    
    elif op == "6":
        print("VENDA DO PRODUTO")
        imprimirLista(produtos)
        
        while True: 
            venda = int(input("Digite o número do produto que você deseja comprar: "))
        
            if venda == 0:
                print("VENDA CANCELADA")
            else:
                if venda >= 1 and venda <= len(produtos):
                    produto_escolhido = produtos[venda - 1]
                    break
                else:
                    print("Digite o número de um produto váilido")
            
                while True:
                    qnt_produto = int(input("Digite a quantidade do produto: "))
            
                    total_venda = produto_escolhido["Preço"] *qnt_produto
            
                    produto_escolhido["Estoque"] -= qnt_produto

                    print(f"""
    --------- RESUMO DA VENDA ---------
    Nome do produto: {produto_escolhido["Nome"]}
